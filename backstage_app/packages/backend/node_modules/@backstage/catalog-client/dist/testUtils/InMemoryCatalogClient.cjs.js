...
function serializeFilterValue(v) {
  if (v === catalogClient.CATALOG_FILTER_EXISTS) return FILTER_EXISTS_SENTINEL;
  if (Array.isArray(v)) {
    return v.map(
      (x) => x === catalogClient.CATALOG_FILTER_EXISTS ? FILTER_EXISTS_SENTINEL : x
    );
  }
  return v;
}
function deserializeFilterValue(v) {
  if (v === FILTER_EXISTS_SENTINEL) return catalogClient.CATALOG_FILTER_EXISTS;
  if (Array.isArray(v)) {
    return v.map(
      (x) => x === FILTER_EXISTS_SENTINEL ? catalogClient.CATALOG_FILTER_EXISTS : x
    );
  }
  return v;
}
  ...
  async getEntities(request) {
    ...
    if (request?.after) {
      try {
        ...
        throw new errors.InputError("Invalid cursor");
      }
    }
    if (offset > 0) {
