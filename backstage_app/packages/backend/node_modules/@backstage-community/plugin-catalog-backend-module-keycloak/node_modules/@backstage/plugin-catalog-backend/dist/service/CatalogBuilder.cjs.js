...
var GenericScmEventRefreshProvider = require('../providers/GenericScmEventRefreshProvider.cjs.js');
var readScmEventHandlingConfig = require('../util/readScmEventHandlingConfig.cjs.js');
...
class CatalogBuilder {
  ...
  async build() {
    ...
    const scmEventHandlingConfig = readScmEventHandlingConfig.readScmEventHandlingConfig(config);
    const locationStore = new DefaultLocationStore.DefaultLocationStore(
      dbClient,
      catalogScmEvents,
      scmEventHandlingConfig
    );
    const configLocationProvider = new ConfigLocationEntityProvider.ConfigLocationEntityProvider(config);
    const scmEvents = new GenericScmEventRefreshProvider.GenericScmEventRefreshProvider(
      dbClient,
      catalogScmEvents,
      scmEventHandlingConfig
    );
    const entityProviderEntries = lodash__default.default.uniqBy(
      [
        ...this.entityProviders,
    ...
    const enabledProviderEntries = util.filterProviders(
    ...
    const processingEngine = new DefaultCatalogProcessingEngine.DefaultCatalogProcessingEngine({
      ...
      orchestrator,
      stitcher,
      createHash: () => node_crypto.createHash("sha1"),
    ...
    const locationService = new AuthorizedLocationService.AuthorizedLocationService(
      new DefaultLocationService.DefaultLocationService(locationStore, orchestrator, {
        ...
        defaultLocationConflictStrategy: config.getOptionalString(
    ...
    await connectEntityProviders.connectEntityProviders(providerDatabase, enabledProviderEntries);
    return {
      processingEngine: {
        async start() {
