...
var parseEntityFacetsQuery = require('./request/parseEntityFacetsQuery.cjs.js');
...
var parseLocationQuery = require('./request/parseLocationQuery.cjs.js');
var parseEntityQuery = require('./request/parseEntityQuery.cjs.js');
...
async function createRouter(options) {
  ...
  if (entitiesCatalog) {
    router$1.get("/entities", async (req, res) => {
      ...
      try {
        const credentials = await httpAuth.credentials(req);
        const { fields: rawFields, ...parsed } = parseEntityQuery.parseEntityQuery(
          req.body ?? {}
        );
        const fields = rawFields?.length ? parseEntityTransformParams.parseEntityTransformParams({ fields: rawFields }) : void 0;
      ...
      try {
        const { facets, query } = parseEntityFacetsQuery.parseEntityFacetsQuery(req.body ?? {});
        const response = await entitiesCatalog.facets({
          query,
          facets,
          credentials: await httpAuth.credentials(req)
  ...
  if (locationService) {
    router$1.post("/locations", async (req, res) => {
      ...
      try {
        const request = parseLocationQuery.parseLocationQuery(req.body ?? {});
        ...
        const items = hasNextPage ? result.items.slice(0, request.limit) : result.items;
        const nextCursor = hasNextPage ? parseLocationQuery.encodeLocationQueryCursor({
          limit: request.limit,
      ...
      try {
        const body = await util.validateRequestBody(
          ...
          v3.z.object({
            ...
            catalogFilename: v3.z.string().optional()
          })
        );
  ...
  router$1.post("/validate-entity", async (req, res) => {
    ...
    try {
      const bodySchema = v3.z.object({
        entity: v3.z.unknown(),
        location: v3.z.string()
      });
      let body;
      let entity;
      let location;
